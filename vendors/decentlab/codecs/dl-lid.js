...
function decodeUplink(input) {
  if (input.hasOwnProperty("variables" && "PARAMETERS" in decentlab_decoder)) {
    for (var key in input.variables) {
      decentlab_decoder.PARAMETERS[key] = input.variables[key];
    }
  }
  return {
    data: decentlab_decoder.decode(input.bytes, input.fPort)
  };
}
...
function encodeDownlink(input) {
  return decentlab_downlink.encode_downlink(input.data.command, input.data.parameter);
}
